...
<HEAD>
	<TITLE>listGoods2 「カテゴリー別商品一覧2」</TITLE>
...
<%
	...
	String reqMaintenanceMode = request.getParameter("maintenanceMode");

	String[] maintenanceModes = {"", "追加", "更新", "削除"};
%>
...
<%
	if(reqGoodsID != null){
%>
		onLoad="window.alert('商品ID「<%= reqGoodsID %>」のレコードが'
				+ '<%= maintenanceModes[Integer.parseInt(reqMaintenanceMode)] %>'
				+ 'されました。')"
<%
	}
%>
...
//	DataSource ds = (DataSource)ic.lookup("java:comp/env/developmentExercise");
//	Connection cn = ds.getConnection();
...
%>
	...
	<TD id="formAddGoods">
		<!-- 商品追加リクエストフォーム -->
		<FORM method="POST" action="addGoods2_.jsp">
...
%>
		...
		<TD id="list" style="width:50px; text-align:center;">
			<A href="updateGoods2_.jsp?goodsID=<%= goodsID %>">更新</A></TD>
		<TD id="list" style="width:50px; text-align:center;">
			<A href="deleteGoods_.jsp?goodsID=<%= goodsID %>">削除</A></TD>
	</TR>
<%
